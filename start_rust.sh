...
if [ "$RUST_START_MODE" = "2" ]; then
	...
	if [ ! -f "/steamcmd/rust/RustDedicated" ]; then
		...
		bash /steamcmd/steamcmd.sh +@sSteamCmdForcePlatformType linux +login $STEAMID +force_install_dir /steamcmd/rust +app_info_update 1 +app_update 258550 validate
		#STEAMCMD_OUTPUT=$(bash /steamcmd/steamcmd.sh +@sSteamCmdForcePlatformType linux +login $STEAMID +force_install_dir /steamcmd/rust +app_info_update 1 +app_update 258550 validate | tee /dev/stdout)
...
echo "Starting Rust.."
if [ "$LOGROTATE_ENABLED" = "1" ]; then
	unbuffer /steamcmd/rust/RustDedicated -batchmode -load $RUST_STARTUP_COMMAND +server.secure "$RUST_SERVER_SECURE" +fps.limit $RUST_SERVER_FPS +server.updatebatch $RUST_SERVER_UPDATEBATCH +server.identity "$RUST_SERVER_IDENTITY" +server.seed "$RUST_SERVER_SEED"  +server.hostname "$RUST_SERVER_NAME" +server.url "$RUST_SERVER_URL" +server.headerimage "$RUST_SERVER_BANNER_URL" +server.description "$RUST_SERVER_DESCRIPTION" +server.worldsize "$RUST_SERVER_WORLDSIZE" +server.maxplayers "$RUST_SERVER_MAXPLAYERS" +server.saveinterval "$RUST_SERVER_SAVE_INTERVAL" 2>&1 | grep --line-buffered -Ev '^\s*$|Filename' | tee $RUST_SERVER_LOG_FILE &
else
	/steamcmd/rust/RustDedicated -batchmode -load -logfile $RUST_STARTUP_COMMAND +server.secure "$RUST_SERVER_SECURE" +fps.limit $RUST_SERVER_FPS +server.updatebatch $RUST_SERVER_UPDATEBATCH +server.identity "$RUST_SERVER_IDENTITY" +server.seed "$RUST_SERVER_SEED"  +server.hostname "$RUST_SERVER_NAME" +server.url "$RUST_SERVER_URL" +server.headerimage "$RUST_SERVER_BANNER_URL" +server.description "$RUST_SERVER_DESCRIPTION" +server.worldsize "$RUST_SERVER_WORLDSIZE" +server.maxplayers "$RUST_SERVER_MAXPLAYERS" +server.saveinterval "$RUST_SERVER_SAVE_INTERVAL" 2>&1 &
fi
